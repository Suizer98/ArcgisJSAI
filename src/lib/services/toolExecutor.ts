...
export class ToolExecutor {
  ...
  static async executeMissingTools(
    ...
    console.log(`Tools called but not all executed - ${toolCalls.length} calls, ${toolResults.length} results. Executing missing ones manually...`)
    
    // Find which tools were already executed
    const executedToolNames = new Set(toolResults.map(result => result.toolName))
    const missingToolCalls = toolCalls.filter(call => !executedToolNames.has(call.toolName))
    
    console.log('Missing tool calls to execute:', missingToolCalls.map(call => call.toolName))
    
    for (const toolCall of missingToolCalls) {
      try {
        console.log('Executing missing tool manually:', toolCall.toolName, toolCall.input)
