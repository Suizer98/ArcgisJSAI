...
export class AIService {
  ...
  async processUserInput(userInput: string): Promise<AIResponse> {
    ...
    try {
      ...
      toolResults = await ToolExecutor.executeMissingTools(toolCalls, toolResults, aiMessages)
      ...
      if (responseText.includes('<') && responseText.includes('>')) {
        console.log('AI returned malformed response with HTML-like tags:', responseText)
        responseText = "I encountered an issue with the response format. Please try asking again."
      }
      
      // If AI didn't provide text but called tools, provide a helpful response
      if (!responseText && toolCalls.length > 0) {
        console.log('AI called tools but provided no text - providing helpful response')
        responseText = "I've performed the requested operations. Here are the results:"
      }
      
      // Process tool calls and results
      responseText = ResponseProcessor.processToolCalls(toolCalls, responseText)
      responseText = ResponseProcessor.processToolResults(toolResults, responseText)

      // Fallback: If geolocation was called but no results, try direct geolocation
      if (toolCalls && toolCalls.length > 0) {
        const geolocationCall = toolCalls.find(call => 
          call.toolName === 'getCurrentLocation' || call.toolName === 'centerOnCurrentLocation'
        )
        
        if (geolocationCall && (!toolResults || toolResults.length === 0)) {
          console.log('Geolocation tool called but no results - trying direct fallback')
          try {
