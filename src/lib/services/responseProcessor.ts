...
export class ResponseProcessor {
  ...
  static processToolResults(toolResults: any[], responseText: string): string {
    ...
    const successfulResults = toolResults.filter(result => result.output?.success)
      ...
      try {
        ...
        if (toolResultData) {
          ...
          if (toolResultData.coordinates) {
            const lat = toolResultData.coordinates.latitude || toolResultData.coordinates.lat
            const lng = toolResultData.coordinates.longitude || toolResultData.coordinates.lng
          ...
          if (toolResultData.location && toolResult.toolName !== 'getCurrentLocation') {
          ...
          } else if (toolResultData.type === 'polygon' && toolResultData.coordinates) {
            responseText += `\n🔷 Polygon: ${toolResultData.coordinates.length} vertices`
          } else if (toolResultData.type === 'circle' && toolResultData.coordinates) {
          ...
          } else if (toolResultData.type === 'rectangle' && toolResultData.coordinates) {
          ...
          } else if (toolResultData.type === 'point' && toolResultData.coordinates) {
          ...
          } else if (toolResultData.type === 'location_with_marker' && toolResultData.coordinates) {
          ...
          if (toolResultData.state && toolResultData.state.center) {
            const stateLat = toolResultData.state.center.latitude || toolResultData.state.center.lat
            const stateLng = toolResultData.state.center.longitude || toolResultData.state.center.lng
          ...
          if (toolResultData.currentState && toolResultData.currentState.center) {
            const currentLat = toolResultData.currentState.center.latitude || toolResultData.currentState.center.lat
            const currentLng = toolResultData.currentState.center.longitude || toolResultData.currentState.center.lng
          ...
          if (!toolResultData.coordinates && !toolResultData.location && !toolResultData.zoomLevel && !toolResultData.state && !toolResultData.currentState && !toolResultData.availableOperations && !toolResultData.results) {
    ...
    const criticalFailures = failedResults.filter(result => {
      ...
      const hasSuccessfulOperation = successfulResults.some(success => success.toolName === toolName)
      // Don't show failures for map operations if we have successful ones
      const isMapOperation = ['setMapCenterAndZoom', 'setMapCenter', 'setMapZoom'].includes(toolName)
      ...
      criticalFailures.forEach((toolResult) => {
        try {
          ...
          if (toolResultData) {
            ...
            if (toolResult.toolName === 'getCurrentLocation' || toolResult.toolName === 'centerOnCurrentLocation') {
              if (toolResultData.message && toolResultData.message.includes('denied')) {
