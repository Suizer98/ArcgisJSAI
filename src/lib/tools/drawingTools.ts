...
function parseColor(colorInput: string): string {
  ...
  if (color.startsWith('rgb(') && color.endsWith(')')) {
    const rgb = color.slice(4, -1).split(',').map(n => parseInt(n.trim()))
...
export const drawLineTool = tool({
  ...
  inputSchema: z.object({
    startLatitude: z.number().min(-90).max(90).describe('The starting latitude coordinate'),
    startLongitude: z.number().min(-180).max(180).describe('The starting longitude coordinate'),
    endLatitude: z.number().min(-90).max(90).describe('The ending latitude coordinate'),
    endLongitude: z.number().min(-180).max(180).describe('The ending longitude coordinate'),
    color: z.string().optional().describe('Optional: Color of the line (supports: color names like "red", "blue", "green", "yellow", "orange", "purple", "pink", "brown", "black", "white", "gray", "cyan", "magenta", "lime", "navy", "maroon", "olive", "teal", "silver", "gold", "crimson", "darkred", "lightblue", "darkblue", "lightgreen", "darkgreen", or hex codes like "#FF0000", or RGB like "rgb(255,0,0)")')
  }),
  execute: async ({ startLatitude, startLongitude, endLatitude, endLongitude, color = "red" }) => {
    try {
      const parsedColor = parseColor(color)
      console.log('Drawing line from', startLatitude, startLongitude, 'to', endLatitude, endLongitude, 'in color', parsedColor)
      
      // Actually draw the line on the map
      const result = await mapController.drawLine(startLatitude, startLongitude, endLatitude, endLongitude, parsedColor)
...
export const drawPointTool = tool({
  ...
  inputSchema: z.object({
    ...
    longitude: z.number().min(-180).max(180).describe('The longitude coordinate'),
    color: z.string().optional().describe('Optional: Color of the point (supports: color names like "red", "blue", "green", "yellow", "orange", "purple", "pink", "brown", "black", "white", "gray", "cyan", "magenta", "lime", "navy", "maroon", "olive", "teal", "silver", "gold", "crimson", "darkred", "lightblue", "darkblue", "lightgreen", "darkgreen", or hex codes like "#FF0000", or RGB like "rgb(255,0,0)")'),
    size: z.number().optional().describe('Optional: Size of the point in pixels'),
  ...
  execute: async ({ latitude, longitude, color = "red", size = 10, label }) => {
    try {
      const parsedColor = parseColor(color)
      console.log('Drawing point at', latitude, longitude, 'in color', parsedColor)
      
      // Actually draw the point on the map
      const result = await mapController.drawPoint(latitude, longitude, parsedColor, size, label)
