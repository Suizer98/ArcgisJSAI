  ...
  execute: async () => {
    try {
      ...
      while (!result.success && retryCount < maxRetries) {
        console.log(`Geolocation attempt ${retryCount + 1} failed, retrying...`)
      ...
      if (result.success && result.coordinates) {
        ...
        try {
          ...
          console.error('Failed to place marker at current location:', markerError)
        }
        ...
        try {
          ...
          console.error('Failed to recenter map to current location:', recenterError)
        }
...
export const centerOnCurrentLocationTool = tool({
  description: 'Center the map on the user\'s current location using geolocation',
  inputSchema: z.object({}),
  execute: async () => {
    try {
