...
export const setMapCenterTool = tool({
  ...
  inputSchema: z.object({
    latitude: z.number().min(-90).max(90).describe('The latitude coordinate (-90 to 90)'),
    longitude: z.number().min(-180).max(180).describe('The longitude coordinate (-180 to 180)'),
    location: z.string().optional().describe('Optional: Human-readable location name')
  }),
...
export const setMapZoomTool = tool({
  ...
  inputSchema: z.object({
    level: z.number().min(1).max(20).describe('The zoom level (1-20, where 1 is world view and 20 is street level)')
  }),
  execute: async ({ level }) => {
    try {
...
export const setMapCenterAndZoomTool = tool({
  ...
  execute: async ({ latitude, longitude, zoom, location }) => {
    try {
      const result = await mapController.setCenterAndZoom(latitude, longitude, zoom)
      return {
        success: result.success,
        message: result.message,
        coordinates: { latitude, longitude },
...
export const getMapStateTool = tool({
  description: 'Get the current map state including center coordinates and zoom level',
  inputSchema: z.object({}),
  execute: async () => {
    try {
...
export const getMapInfoTool = tool({
  description: 'Get information about the current map state and available operations',
  inputSchema: z.object({}),
  execute: async () => {
    try {
      const state = mapController.getCurrentState()
